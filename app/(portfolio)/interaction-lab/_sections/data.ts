...
export const researches: ResearchRecord[] = [
  ...
  // },
  {
    ...
    summary:
      "양손 엄지로 플레이하는 모바일 사용자를 고려해, 동작 분류에 따라 좌우로 분리 배치했다. 메인 동작에 해당하는 색상 팔레트는 왼쪽에, 풀기&툴 변경 등 부가 동작 버튼은 오른쪽에 배치해 양쪽 엄지의 역할이 명확히 구분되도록 했다.",
    methods: [
  ...
  },
  {
    ...
    summary:
      "뜨개뜨개는 색상 변경 타이밍 파악, 실패 코 인지, 하드 모드에서는 코 종류까지 동시에 처리해야 하는 멀티태스킹 상황이다. 처리해야 할 정보가 많아질수록 시각 채널만으로는 실패를 제때 인지하기 어렵다고 판단해 실패 효과음을 추가했다.",
  ...
  },
  {
    ...
    summary:
      "뜨개뜨개의 기본 팔레트는 파스텔 계열 색상으로 구성되어 있어, 색각 이상(색약·색맹) 사용자가 색상을 구분하기 어렵다는 문제가 있었다. 색각 이상 환경에서도 혼동이 적은 Wong palette로 대체 팔레트를 설계하고, 모드 선택 화면에서 토글로 전환할 수 있는 색약 모드를 구현했다.",
    methods: [
